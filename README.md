...
This will open a pop-up window. From there, you need to click on the blue "Install" icon next to the **Recommended" Unity version as shown in the picture below - 
...
Note that, "Visual Studio Code" will be downloaded automatically. Let the installation finish first. Once it is finished, we will set up "Visual Studio Code" for Unity so that you can write scripts and compile it in Unity flawlessly. So, turn on "Visual Studio Code" because we need to install some extensions in the software now. 

Click on the Extensions icon on the left side bar of your Visual Studio Code, and download these extensions one by one -
